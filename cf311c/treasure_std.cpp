...
{
	long long h_;
	freopen("treasure.in","r",stdin), freopen("treasure.ans","w",stdout);
	scanf("%I64d%d%d%d",&h_,&n,&m,&k); memset(d,1,sizeof(d)),d[1]=0;
	rep(i,n) {scanf("%I64d%d",a+i,c+i); if (a[i]%k==1) H.push(mk(c[i],-i));}
	while (m--){
		scanf("%d",&type);
		while (type==3){
		...
		if (type==1){
			scanf("%I64d",&z); data[++L]=z/k,ed[L]=z%k;			
