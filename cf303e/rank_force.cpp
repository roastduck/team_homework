...
int main()
{
	...
	{
		int ll, rr;
		ll=lower_bound(sep,sep+n,L[t])-sep;
		rr=lower_bound(sep,sep+n,R[t])-sep;
		for (int x=ll;x<rr;x++)
		{
			for (int ii=1,i=0;ii<=N;ii++) if (ii!=t)
				Push(x,i++,ii);
			double p=double(sep[x+1]-sep[x])*reci[t];
			for (int i=0;i<=N-1;i++)
				for (int j=0;j<=N-1-i;j++)
				{
					ans[t][i+1]+=f[N-1][i][j]*p/(j+1);
					ans[t][i+j+2]-=f[N-1][i][j]*p/(j+1);
				}
		}
	}
	for (int i=1;i<=N;i++)
